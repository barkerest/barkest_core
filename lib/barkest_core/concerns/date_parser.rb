  ...
  module DateParser
    ...
    def self.parse_for_date_column(value)
      ...
      if parts
        ...
        begin
          ...
          # some dates, like 2000-02-30 are still generated by the Time class.
          # we don't want to generate those dates, so instead, check that the parts all match what we wanted.
          raise ArgumentError, 'date does not match' unless value.year == y && value.month == m && value.day == d
    ...
    def self.parse_for_time_column(value)
      ...
      if parts
        ...
        begin
          t = t.to_s.upcase
          raise ArgumentError, 'hour must be between 1 and 12 for AM/PM notation' unless t.blank? || (1..12).include?(h)
          h += 12 if t == 'PM' and h != 12
          h = 0 if t == 'AM' and h == 12

          # allow 24:00:00 to roll over.
          if h == 24 && n == 0 && s == 0
            value = Time.zone.local(y, m, d)
            raise ArgumentError, 'date does not match' unless value.year == y && value.month == m && value.day == d
          ...
          # some dates, like 2000-02-30 are still generated by the Time class.
          # we don't want to generate those dates, so instead, check that the parts all match what we wanted.
          raise ArgumentError, 'date/time does not match' unless value.year == y && value.month == m && value.day == d && value.hour == h && value.min == n && value.sec == s
        rescue ArgumentError
          value = nil
        end
